...
exports.Post = async (req, res) => {
  let sqlcherch3 =
    "SELECT * FROM `language` WHERE `userid` =" + mysql.escape(req.body.userid);
  ...
  con.query(sqlcherch3, (err, find1) => {
    ...
    for (let index = 0; index < find1.length; index++) {
      ...
      if (element.lang != "") {
        arr.push(element.lang);
      }
    }
    if (req.body.language != undefined) {
      req.body.language.forEach((languages) => {
        console.log(languages);
        if (arr.indexOf(languages.lang) < 0) {
          let languagey = {
            lang: languages.lang,
          ...
          con.query(sql, languagey, (err, sqlres) => {
            if (err) throw err;
            console.log(sqlres);
          });
        } else {
